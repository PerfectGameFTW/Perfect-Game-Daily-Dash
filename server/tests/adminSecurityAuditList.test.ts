...
describe('GET /admin/security/audit (Task #126)', () => {
  ...
  // The actor admin is TOTP-enrolled (see beforeAll), so the login flow
  // produces audit rows (totp.login_success, etc. — Task #131). The
  // test body asserts on exact row counts, so we log in EXACTLY ONCE
  // per suite and cache the cookie. beforeEach scrubs every audit row
  // for the actor before seeding the test fixtures, which cleans up
  // both the enrollment rows from beforeAll and the login_success row
  // generated by this one-time login.
  let actorCookie: string | null = null;
  ...
  beforeAll(async () => {
    ...
    // Real TOTP enrollment for the admin actors. The mandatory-2FA gate
    // (Task #100) toggle is in app_settings, which is shared across all
    // test files via the same DB. Sibling suites — most notably
    // adminTwoFactor.test.ts — flip that toggle on mid-run, so the
    // safest cure for flaky 403 responses here is to make the audit
    // admin actually 2FA-enrolled and have the login helper complete
    // the TOTP step. With totpEnabled=true AND a fully-verified
    // session, requireAuth's gate is satisfied either way.
    actorTotpSecret = (
      await totpService.beginEnrollment(a, { ip: '127.0.0.1' })
    ).secret;
    await totpService.verifyAndEnable(
      actorId,
      currentCode(actorTotpSecret, ADMIN_AUDIT_ACTOR),
      { ip: '127.0.0.1' },
    );
    ...
    // Pre-warm the actor's authenticated session NOW so the very first
    // beforeEach (which deletes every actor audit row before seeding the
    // 3 test fixtures) also clears the totp.login_success row that this
    // login produces. If we let the first test trigger the login
    // instead, that row would land between beforeEach's scrub and the
    // test's assertion and skew the row count by one.
    await login(ADMIN_AUDIT_ACTOR);
  });
  ...
  async function login(username: string): Promise<string> {
    // The admin actor's session is reusable across tests: nothing in
    // beforeEach destroys server-side session rows, and caching avoids
    // re-driving the TOTP flow (which would produce new audit rows
    // mid-test and confound row-count assertions).
    if (username === ADMIN_AUDIT_ACTOR && actorCookie) return actorCookie;
    ...
    // For the TOTP-enrolled actor admin, login returns a pending
    // session that has to be graduated by submitting the current
    // authenticator code. NORMAL_USER is unenrolled so its login is
    // fully authenticated already and skips this step.
    if (r.body?.requiresTotp) {
      const v = await jsonReq(
        `${baseUrl}/api/auth/totp/verify`,
        'POST',
        { code: currentCode(actorTotpSecret, ADMIN_AUDIT_ACTOR) },
        cookie,
      );
      expect(v.status).toBe(200);
      if (v.cookie) cookie = v.cookie.split(';')[0];
    }
    if (username === ADMIN_AUDIT_ACTOR) actorCookie = cookie;
    return cookie;
  }
