  ...
  try {
    ...
    for (const giftCard of giftCardTransactions.rows) {
      try {
        ...
        // Update the gift card with the activation amount from the order
        await db.update(giftCards)
          .set({ activationAmount: Number(giftCardAmount) })
          .where(sql`id = ${giftCardId}`);
        
        console.log(`✅ Updated gift card ID ${giftCardId} with activation amount $${Number(giftCardAmount).toFixed(2)} from order ID ${closestOrder.order_id}`);
        updatedCount++;
      } catch (error) {
