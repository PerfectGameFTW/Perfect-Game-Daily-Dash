...
- **Reset link base URL is operator-pinned, not Host-header derived**: In production (`NODE_ENV=production`) `APP_BASE_URL` MUST be set; otherwise the email is suppressed (with an operator alarm in the log) rather than constructed from a potentially attacker-supplied `Host` header — which would let an attacker phish a victim's reset token by poisoning the link domain. In dev the request host is used as a convenience fallback. Email delivery uses the Replit SendGrid integration (preferred — credentials and verified sender are pulled from the connectors API at send time) or the raw `SENDGRID_API_KEY` env var; in dev, with neither configured, the message is logged to the workflow console. In production the email service throws if SendGrid is not wired (no silent drops). `MAIL_FROM_EMAIL` optionally overrides the connector's verified sender; `MAIL_FROM_NAME` controls the display name.
